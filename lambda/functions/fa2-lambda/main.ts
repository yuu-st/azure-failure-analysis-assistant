...
export async function httpTrigger(req: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
  ...
  */
  ...
  try {
    ...
    const storageAccountName : string = process.env["STORAGE_ACCOUNT_NAME"] ? process.env["STORAGE_ACCOUNT_NAME"] : "";
    const containerName : string = process.env["CONTAINER_NAME"] ? process.env["CONTAINER_NAME"] : "";
    ...
    const azureOpenAIEndpoint: string = process.env["AZURE_OPENAI_ENDPOINT"] ? process.env["AZURE_OPENAI_ENDPOINT"] : "";
    const azureOpenAIKey: string = process.env["AZURE_OPENAI_KEY"] ? process.env["AZURE_OPENAI_KEY"] : "";
